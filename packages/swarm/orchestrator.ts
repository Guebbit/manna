...
export class SwarmOrchestrator {
    ...
    async run(task: string, config: ISwarmConfig = {}): Promise<ISwarmResult> {
        ...
        const subtaskResults = await this.executeSubtasks(decomposition, config);
    ...
    private async executeSubtasks(
        ...
        const remaining = new Map<string, ISubtask>(decomposition.subtasks.map((s) => [s.id, s]));
        ...
        while (remaining.size > 0) {
            ...
            for (const subtask of ready) {
                const result = await this.executeOneSubtask(subtask, results, config);
                results.push(result);
     ...
     */
    private buildSubtaskPrompt(subtask: ISubtask, previousResults: ISubtaskResult[]): string {
