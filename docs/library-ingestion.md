...
**When to prefer full RAG instead:** if you need the system to *synthesise* an answer from across multiple articles (e.g. "summarise 10 years of coverage on CRISPR"), the summary-index approach won't serve you — you'd need to load full article text into the LLM prompt. For pure *discovery* ("which articles cover X?"), the summary index is superior.
...
**Why two passes?**
- Pass 1 is fast (TOC pages only) and produces the *structure* used by Pass 2.
